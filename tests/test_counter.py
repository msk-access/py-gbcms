...
def test_count_variant_dispatch(counter):
    ...
    snp_variant = VariantEntry(
        chrom="chr1", pos=5, end_pos=5, ref="A", alt="T", snp=True
    )
    ...
    # Insertion
    ins_variant = VariantEntry(
        chrom="chr1", pos=5, end_pos=5, ref="A", alt="AT", insertion=True
    )
    ins_variant.initialize_counts(["sample1"])
    counter.count_variant(ins_variant, [], "sample1")
    
    # Deletion
    del_variant = VariantEntry(
        chrom="chr1", pos=5, end_pos=6, ref="AT", alt="A", deletion=True
    )
