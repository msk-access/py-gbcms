    ...
    def generate(self, config, sample_order: list[str]) -> list[str]:
        ...
        for count_type in self.count_types:
            ...
            if config.output_strand_count:
                columns.extend(
                    [f"{self.prefix}{count_type}_forward", f"{self.prefix}{count_type}_reverse"]
                )
    ...
    def generate(self, config, sample_order: list[str]) -> list[str]:
        ...
        for sample in sample_order:
            for template in self.column_templates:
                ...
                if config.output_strand_count:
                    columns.extend([f"{sample}:{template}_FORWARD", f"{sample}:{template}_REVERSE"])
...
class TemplateEngine:
    ...
    def _initialize_default_templates(self):
        ...
        vcf_template = [
            ...
            SampleSpecificColumnTemplate(["DP", "RD", "AD", "VAF"]),
        ]
        ...
        maf_base_template = [
            ...
            CountColumnTemplate("t_", ["depth", "ref_count", "alt_count", "vaf"]),
        ]
