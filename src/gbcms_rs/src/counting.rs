...
fn count_single_variant(
    ...
    let tid = bam.header().tid(variant.chrom.as_bytes()).ok_or_else(|| {
        PyErr::new::<pyo3::exceptions::PyValueError, _>(format!("Chromosome not found in BAM: {}", variant.chrom))
    })?;
    ...
    for result in bam.records() {
        let record = result.map_err(|e| {
            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("Error reading BAM record: {}", e))
        })?;

        // Apply filters
        if filter_duplicates && record.is_duplicate() { continue; }
        if filter_secondary && record.is_secondary() { continue; }
        if filter_supplementary && record.is_supplementary() { continue; }
    ...
    for (qname, orientations) in fragment_orientations {
        ...
        if has_ref {
            counts.rdf += 1;
            if orientation { counts.rdf_fwd += 1; } else { counts.rdf_rev += 1; }
        }
        
        if has_alt {
            counts.adf += 1;
            if orientation { counts.adf_fwd += 1; } else { counts.adf_rev += 1; }
        }
    }
    
    // Calculate stats
    let (sb_pval, sb_or) = fisher_strand_bias(counts.rd_fwd, counts.rd_rev, counts.ad_fwd, counts.ad_rev);
    counts.sb_pval = sb_pval;
    counts.sb_or = sb_or;
    
    let (fsb_pval, fsb_or) = fisher_strand_bias(counts.rdf_fwd, counts.rdf_rev, counts.adf_fwd, counts.adf_rev);
...
fn check_snp(record: &Record, variant: &Variant, min_baseq: u8) -> (bool, bool) {
    ...
    if qual < min_baseq { return (false, false); }

    let base = record.seq()[read_pos] as char;
    let ref_char = variant.ref_allele.chars().next().unwrap().to_ascii_uppercase();
    let alt_char = variant.alt_allele.chars().next().unwrap().to_ascii_uppercase();
