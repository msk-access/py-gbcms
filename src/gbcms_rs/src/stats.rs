use statrs::distribution::{Hypergeometric, DiscreteCDF, Discrete};
...
pub fn fisher_strand_bias(ref_fwd: u32, ref_rev: u32, alt_fwd: u32, alt_rev: u32) -> (f64, f64) {
    ...
    let min_a = if row1_sum + col1_sum > n { row1_sum + col1_sum - n } else { 0 };
    let max_a = if row1_sum < col1_sum { row1_sum } else { col1_sum };
    
    for k in min_a..=max_a {
        let p = dist.pmf(k);
        if p <= p_observed + 1e-10 { // Add epsilon for float comparison
            p_value += p;
        }
    }
