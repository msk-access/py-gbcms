...
=== "Docker"
    ```bash
    docker pull ghcr.io/msk-access/py-gbcms:2.6.1
    docker run --rm ghcr.io/msk-access/py-gbcms:2.6.1 gbcms --help
    ```
...
=== "Singularity (HPC)"
    ```bash
    singularity pull docker://ghcr.io/msk-access/py-gbcms:2.6.1
    singularity exec py-gbcms_2.6.1.sif gbcms --help
    
    # With data binding
    singularity exec -B /path/to/data:/data py-gbcms_2.6.1.sif gbcms run \
...
=== "Docker"
    ```bash
    docker pull ghcr.io/msk-access/py-gbcms:2.6.1
    docker run --rm -v $(pwd):/data ghcr.io/msk-access/py-gbcms:2.6.1 gbcms --help
    ```
...
docker run --rm \
  -v $(pwd):/data \
  ghcr.io/msk-access/py-gbcms:2.6.1 \
  gbcms run \
